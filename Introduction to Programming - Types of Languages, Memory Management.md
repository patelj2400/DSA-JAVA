...
The video explains that languages can follow multiple paradigms.A language is not restricted to just one type.
...
Step 1: Take first numberStep 2: Take second numberStep 3: Add themStep 4: Print result
...
Example:You are an object of class Human.
...
`   class Student {     name     rollNumber     marks  }   `
...
`   int a = 10;  a = "kunal";   // Error   `

Why?Because a was declared as integer.
...
In dynamic typing:Variable can point to different objects over time.
...
Static typing → checked at compile timeDynamic typing → checked at runtime
...
Important:a is NOT 10a is just a reference.
...
Your body = objectYour name = reference variable
...
If object changes (e.g., haircut),All references see same change.
...
In Java:Garbage Collector automatically removes unused objects.
